...
const string GET_LINECOVERAGE_SNAPSHOT_ID="SELECT * FROM line_coverage_snapshot  ORDER BY snapshot_id DESC LIMIT 1";

const string DELETE_LIVE_LINE_COVERAGE_DETAILS="DELETE FROM live_line_coverage WHERE 1";

const string INSERT_LIVE_LINE_COVERAGE_DETAILS = "INSERT INTO live_line_coverage (date,component_id,component_name,lines_to_cover," +
                                                 "covered_lines,uncovered_lines,line_coverage) VALUES (?,?,?,?,?,?,?)";

const string GET_ALL_COMPONENTS="SELECT pqd_component_id,pqd_component_name FROM product_quality_dashboard.pqd_component";

const string GET_FOLDER_OF_COMPONENT="SELECT Folder FROM product_quality_dashboard.JNKS_COMPONENTFOLDER where Component=?";
